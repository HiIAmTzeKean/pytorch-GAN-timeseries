...
from models.convolutional_models import CausalConvDiscriminator, CausalConvGenerator
from models.recurrent_models import LSTMDiscriminator, LSTMGenerator
from utils import time_series_to_plot


parser = argparse.ArgumentParser()
parser.add_argument("--dataset", default="D05.SI", help="dataset to use")
...
parser.add_argument(
    "--alternate", action="store_true", help="whether to alternate between adversarial and mse loss in generator"
)
parser.add_argument(
    "--dis_type", default="cnn", choices=["cnn", "lstm"], help="architecture to be used for discriminator to use"
)
parser.add_argument(
    "--gen_type", default="lstm", choices=["cnn", "lstm"], help="architecture to be used for generator to use"
)
...
def main():
    ...
    dataset = StockDataset(ticker=opt.dataset)
    ...
    if opt.gen_type == "cnn":
        netG = CausalConvGenerator(
            noise_size=in_dim, output_size=1, n_layers=8, n_channel=10, kernel_size=8, dropout=0.2
        ).to(device)

    if netG is None:
        raise ValueError(f"No generator found for type {opt.gen_type}")
    if netD is None:
        raise ValueError(f"No discriminator found for type {opt.dis_type}")
